...
function StartDisplayBarVertical(percentages, colors, names, baliseId){
...
function DisplayScale(context, strongPercentage, height, width, fontSize){
	context.beginPath();
	context.moveTo(width * 0.075, height * 0.1);
	context.lineTo(width * 0.075, height * 0.95);
	context.stroke();
...
function DisplayStepScale(context, stepScale, strongPercentage, height, width, fontSize){
	context.beginPath();
	context.moveTo(width * 0.025, height * 0.1);
	...
	for(var i = numberSteps; i>0; i--){
		if (i*stepScale <= strongPercentage)
			context.fillText(i * stepScale, width * 0.025, height * 0.85 - (height * 0.85 * i) / (strongSteps) + height * 0.10 - height * 0.01);
	}
	context.stroke();
	...
	for(var i = numberSteps; i>0; i--){
		if (i*stepScale <= strongPercentage){
			context.beginPath();
			context.moveTo(width * 0.025, height * 0.85 - (height * 0.85 * i) / (strongSteps) + height * 0.1);
			context.lineTo(width * 0.075, height * 0.85 - (height * 0.85 * i) / (strongSteps) + height * 0.10);
			context.stroke();
		}
	}
}
...
function DisplayPercentage(percentage, strongPercentage, color, context, width, height, i, count, fontSize){
	...
	var heightStart = height * 0.1 + height * 0.85 * (1 - (1 / (strongPercentage / percentage)));
	var heightEnd = height * 0.85 * 1 / (strongPercentage / percentage);
	var widthStart = width * 0.1 + ((width * 0.9) / count) * i;
	var widthEnd = ((width * 0.85) / count);
	...
	context.fillText(percentage, widthStart + widthEnd / 2, heightStart - height * 0.025);
} 
